...
@Repository
public interface MusicFileStorageInDataBaseRepository extends JpaRepository<MusicFileStoredInDataBase, Long> {
    //@Modifying
    ...
    @Query(value="SELECT * FROM music_files_database INNER JOIN questions ON  music_files_database.question_id = questions.question_id;",nativeQuery = true)//this syntax is correct

    //@Query(value="select f from files_database f where f.questionId = :questionId",nativeQuery = true)//this syntax is correct
    List<MusicFileStoredInDataBase> findByQuestionId(@Param("questionId") Long questionId);
}